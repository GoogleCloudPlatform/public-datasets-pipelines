...
def main(
    ...
    logging.info(f"Transform: Rename columns...")
    rename_headers(df, rename_mappings)

    logging.info(f"Transform: Cleaning up column location_description...")
    df["location_description"] = df["location_description"].astype('|S').str.decode("utf-8").apply(reg_exp_tranformation,args=(r"\s{2,}",""))

    logging.info(f"Transform: Converting to integer string...")
    df['zipcode'] = df['zipcode'].apply(convert_to_integer_string)
    df['council_district_code'] = df['council_district_code'].apply(convert_to_integer_string)
    ...
    logging.info(f"Transform: Creating a new column - address...")
    df['address'] = df['address1']
    df['address'] = df['address1'].fillna(df['location_description'] + ' Austin, TX ' + df['zipcode'])

    logging.info(f"Transform: Converting date format...")
    df['timestamp'] = df['timestamp'].apply(convert_dt_format)
    df['clearance_date'] = df['clearance_date'].apply(convert_dt_format)

    logging.info(f"Transform: Creating a new column - year...")
    df['year'] = df['timestamp'].apply(extract_year)

    logging.info(f"Transform: Replacing values...")
    df["address"] = df["address"].apply(reg_exp_tranformation,args=(r"\n"," "))
    df = df.replace(to_replace={
        'clearance_status' : {'C' : 'Cleared by Arrest','O' : 'Cleared by Exception','N' : 'Not cleared'},
        'address' : {'sAustin' : 'Austin'}
    })

    logging.info(f"Transform: Converting exponential values to integer...")
    df['unique_key'] = df['unique_key'].apply(convert_exp_to_float).astype(float).apply(convert_to_integer_string)

    logging.info(f"Transform: Creating a new column - latitude...")
    df['latitude'] = df['address'].apply(search_string).apply(extract_lat_long,args=[r'.*\((\d+\.\d+),.*'])

    logging.info(f"Transform: Creating a new column - longitude...")
    df['longitude'] = df['address'].apply(search_string).apply(extract_lat_long,args=[r'.*(\-\d+\.\d+)\)'])

    logging.info(f"Transform: Creating a new column - year...")
    df['location'] = '(' + df['latitude'] + ',' + df['longitude'] + ')'

    logging.info(f"Transform: Dropping column - address1...")
    ...
    logging.info(
        f"Austin crime process completed at "
        + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    )

def download_file(source_url: typing.List[str], source_file: typing.List[pathlib.Path]) -> None:
...
def convert_dt_format(dt_str: str) -> str:
    ...
    if dt_str is None or len(str(dt_str)) == 0 or str(dt_str) == "nan":
        ...
        return datetime.datetime.strptime(str(dt_str), "%m/%d/%Y %H:%M:%S %p").strftime( "%Y-%m-%d %H:%M:%S" )
