...
with DAG(
    dag_id="epa_historical_air_quality.no2_hourly_summary",
    ...
    create_cluster = kubernetes_engine.GKECreateClusterOperator(
        ...
        body={
            "name": "epa-hist-air-quality--no2-hourly",
    ...
    transform_csv_and_load_data = kubernetes_engine.GKEStartPodOperator(
        ...
        project_id="{{ var.value.gcp_project }}",
        location="us-central1-c",
        cluster_name="epa-hist-air-quality--no2-hourly",
        image_pull_policy="Always",
        image="{{ var.json.epa_historical_air_quality.container_registry.run_csv_transform_kub }}",
        env_vars={
            "SOURCE_URL": "{{ var.json.epa_historical_air_quality.no2_hourly.source_url }}",
            "START_YEAR": "1990",
            "SOURCE_FILE": "files/data.csv",
            "TARGET_FILE": "files/data_output.csv",
            "PROJECT_ID": "{{ var.value.gcp_project }}",
            "DATASET_ID": "{{ var.json.epa_historical_air_quality.dataset_id }}",
            "TABLE_ID": "{{ var.json.epa_historical_air_quality.no2_hourly.table_id }}",
            "SCHEMA_PATH": "{{ var.json.epa_historical_air_quality.no2_hourly.schema_path }}",
            "CHUNKSIZE": "{{ var.json.epa_historical_air_quality.no2_hourly.chunk_size }}",
            "TARGET_GCS_BUCKET": "{{ var.value.composer_bucket }}",
            "TARGET_GCS_PATH": "{{ var.json.epa_historical_air_quality.no2_hourly.target_gcs_path }}",
