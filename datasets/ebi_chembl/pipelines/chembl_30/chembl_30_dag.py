...
with DAG(
    ...
    start_db_instance = bash.BashOperator(
        ...
        env={"instance_id": "{{ var.json.ebi_chembl.instance_id }}"},
    )
    ...
    activities_transform_csv = kubernetes_pod.KubernetesPodOperator(
        ...
        env_vars={
            "INSTANCE_CONNECTION_NAME": "{{ var.json.ebi_chembl.instance_connection_name }}",
            "USER": "{{ var.json.ebi_chembl.user }}",
            "PASSWORD": "{{ var.json.ebi_chembl.password }}",
            "DATABASE": "{{ var.json.ebi_chembl.database }}",
            "TABLES": '["activities"]',
            "CHUNK_SIZE": "100000",
    ...
    # Run CSV transform within kubernetes pod
    compound_structures_transform_csv = kubernetes_pod.KubernetesPodOperator(
        task_id="compound_structures_transform_csv",
        startup_timeout_seconds=600,
    ...
    # Run CSV transform within kubernetes pod
    rem_all_transform_csv = kubernetes_pod.KubernetesPodOperator(
        task_id="rem_all_transform_csv",
        startup_timeout_seconds=600,
        name="rem_all",
        namespace="composer",
        service_account_name="datasets",
    ...
    # Task to 'stop' CloudSQL Database Instance.
    stop_db_instance = bash.BashOperator(
    ...
    (
        ...
        >> [
            ...
            compound_structures_transform_csv,
            rem_all_transform_csv,
        ]
        >> stop_db_instance
    )
    activities_transform_csv >> load_activities_to_bq
    compound_structures_transform_csv >> load_compound_structures_to_bq
    rem_all_transform_csv >> [
