...
def main(
    ...
    for name, table_dat in list(zip(table_name, table_dat)):
        ...
        logging.info(f"uploading output file to... gs://{target_gcs_bucket}/data/{name}.csv")
        upload_to_bucket(bucket_name=target_gcs_bucket, file_name=name, data=str(csv_data))
...
def generate_products():
    ...
    for val in list(zip(product_id, brands, name, cost, category, department, sku, retail_price, distribution_center_id)):
        product_by_id_dict[val[0]] =  ({"brand": val[1], "name": val[2], "cost": val[3], 
                                        "category": val[4], "department": val[5], "sku": val[6], 
                                        "retail_price": val[7], "distribution_center_id": val[8]})
    ...
    for val in list(zip(product_id, brands, category, department)):
        product_id_dict[val[0]] = {"brand": val[1], "category": val[2], "department": val[3]}
...
def generate_distribution_centers():
    ...
    with open("data/distribution_centers.csv", encoding="utf-8") as distributioncenterscsv:
...
def get_address(*, country='*',state='*',postal_code='*'):
    ...
    if postal_code != '*':
        if type(postal_code) == str:
            universe += list(filter(lambda row: row['postal_code'] == postal_code, location_data))
        elif type(postal_code) == dict:
            universe += list(filter(lambda row: row['postal_code'] in postal_code.keys(), location_data))
    if state != '*':
        if type(state) == str:
            universe += list(filter(lambda row: row['state'] == state, location_data))
        elif type(state) == dict:
            universe += list(filter(lambda row: row['state'] in state.keys(), location_data))
    if country != '*':
        if type(country) == str:
            universe += list(filter(lambda row: row['country'] == country, location_data))
        elif type(country) == dict:
            universe += list(filter(lambda row: row['country'] in country.keys(), location_data))
    if len(universe) == 0:
...
def created_at(start_date):
    ...
    created_at = start_date + timedelta(days=random_number_of_days) + timedelta(minutes=randrange(1139))
    return created_at
...
def generate_uri(event, product):
    if event == "product":
        ...
        return "/" + event + "/" + product[5].lower() + "/category/" + product[4].lower().replace(" ", "") + "/brand/" + product[1].lower().replace(" ", "")
    else:
        return "/" + event
...
#utility class
class DataUtil:
    def child_created_at(self, probability="uniform"): #returns a random timestamp between now and parent date
        time_between_dates = datetime.now() - self.parent.created_at
        days_between_dates = time_between_dates.days
        if days_between_dates <= 1:
            days_between_dates = 2
        random_number_of_days = random.randrange(1,days_between_dates) #generates random day between now and when user initially got created
        created_at = self.parent.created_at + timedelta(days=random_number_of_days)
        return created_at
    def random_item(self, population, **distribution): #returns single random item from a list based off distribution
        if distribution:
            return random.choices(population=population, weights=distribution["distribution"])[0]
        else:
...
class Users(DataUtil):
    ...
    def __post_init__(self):
        ...
        if self.gender == "M":
            ...
            self.traffic_source = self.random_item(population=["Organic", "Facebook", "Search", "Email", "Display"], distribution=[.15, .06, .7, .05, .04])
        if self.gender == "F":
        ...
        self.email = self.first_name.lower() + self.last_name.lower() + "@" +  fake.free_email_domain()
        year = datetime.now().year
        month = datetime.now().month
        day = datetime.now().day
        
        #weight newer users/orders
        choice = random.choices([0,1], weights=[.975,.025])[0]
        if choice == 0:
            self.created_at = created_at(datetime(2019,1,1))
        if choice == 1:
            self.created_at = created_at(datetime.now() - timedelta(days=7))
        num_of_orders = random.choices(population=[0,1,2,3,4], weights = [.2, .5, .2, .05, .05])[0]
        if num_of_orders == 0:
            pass
        else:
...
class Product:
    ...
    def __post_init__(self):
        person = Person()
        random_idx = np.random.choice(a=len(product_gender_dict[person.gender]), size = 1)[0]
...
class Order(DataUtil):
    ...
    def __post_init__(self, user=None):
        ...
        self.status = self.random_item(population=["Complete", "Cancelled", "Returned"], distribution=[.85, .05, .1])
        self.created_at = self.child_created_at()
        ##########add random generator for days it takes to ship, deliver, return etc.
        if self.status == "Returned":
            self.shipped_at = self.created_at + timedelta(minutes=randrange(4320)) #shipped between 0-3 days after order placed
            self.delivered_at = self.shipped_at + timedelta(minutes=randrange(7200)) #delivered between 0-5 days after ship date
            self.returned_at = self.delivered_at + timedelta(minutes=randrange(4320)) #returned 0-3 days after order is delivered
        ...
        num_of_items = self.random_item(population=[1,2,3,4], distribution=[.7, .2, .05, .05])
...
class Order_Item(DataUtil):
    ...
    def __post_init__(self, order=None):
        ...
        self.created_at = order.created_at - timedelta(seconds=randrange(14400)) #order purchased within 4 hours
        
        self.shipped_at = order.shipped_at
        self.delivered_at = order.delivered_at
        self.returned_at = order.returned_at

        random_idx = np.random.choice(a=len(product_gender_dict[order.gender]), size = 1)[0]
        ...
        self.browser = self.random_item(population=["IE", "Chrome", "Safari", "Firefox", "Other"], distribution=[0.05, 0.5, 0.2, 0.2, 0.05])
        self.traffic_source = self.random_item(population=["Email", "Adwords", "Organic", "YouTube", "Facebook"], distribution=[0.45, 0.3, 0.05, 0.1, 0.1])
        ...
        if order.num_of_item == 1: #if only 1 item in order go through flow
            for idx, val in enumerate(["home", "department", "product", "cart", "purchase"]):
                ...
                self.created_at = previous_created_at + timedelta(seconds=randrange(180))
        ...
        num_of_items = self.random_item(population=[1, 2, 3], distribution=[.5,.3,.2])
        for i in range(num_of_items):
            self.is_sold = False
            inv_item_id += 1
            self.inventory_item_id = inv_item_id
...
class Inventory_Item:
    ...
    def __post_init__(self, order_item=None):
        ...
        if order_item.is_sold is True:
            self.created_at = order_item.created_at - timedelta(minutes=randrange(86400)) #in inventory between 0 and 60 days
            self.sold_at = order_item.created_at #sold on the date/time the order_items was logged
        ...
        self.product_distribution_center_id = product_by_id_dict[self.product_id]["distribution_center_id"]
    def __str__(self):
...
class Ghost_Events(DataUtil):
    ...
    def __post_init__(self):
        ...
        products = product_gender_dict[self.random_item(population=["M", "F"], distribution=[0.5, 0.5])]
        ...
        random_events = self.random_item(population=[cancelled_browsing, abandoned_cart, viewed_product, viewed_department])
...
if __name__ == "__main__":
    ...
    main(num_of_users=os.environ["NUM_OF_USERS"], target_gcs_bucket=os.environ["TARGET_GCS_BUCKET"], extraneous_headers=json.loads(os.environ["EXTRANEOUS_HEADERS"]))
