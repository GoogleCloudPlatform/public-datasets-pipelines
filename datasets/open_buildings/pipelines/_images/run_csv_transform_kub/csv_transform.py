...
def main(source_gcs_path, project_id, dataset_id, gcs_bucket, schema_filepath):
    ...
    logging.info(
        "All of the following script will be executed for each file from the list of source csv files in gcs"
    )
...
def fetch_gcs_file_names(source_gcs_path, gcs_bucket):
    ...
    logging.info(f"{len(source_file_names)} csv files to undergo ETL from gcs to bq")
