...
with DAG(
    dag_id="fda_drug.drug_enforcement",
    ...
    transform_csv = kubernetes_pod.KubernetesPodOperator(
        ...
        env_vars={
            ...
            "DATE_FORMAT_LIST": [
                ["center_classification_date", "%Y%m%d", "%Y-%m-%d %H:%M:%S"],
                ["report_date", "%Y%m%d", "%Y-%m-%d %H:%M:%S"],
                ["termination_date", "%Y%m%d", "%Y-%m-%d %H:%M:%S"],
                ["recall_initiation_date", "%Y%m%d", "%Y-%m-%d %H:%M:%S"],
            ],
        },
    ...
    load_to_bq = gcs_to_bigquery.GCSToBigQueryOperator(
        ...
        source_format="CSV",
        destination_project_dataset_table="{{ var.json.fda_drug.container_registry.drug_enforcement_destination_table }}",
        skip_leading_rows=1,
