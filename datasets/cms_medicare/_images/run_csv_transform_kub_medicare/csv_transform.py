...
def main(
    ...
    #use fnmatch.filter to get the txt file
        txt_file = fnmatch.filter(file_list, "*.txt")

    #get the csv data
        data = zipped_files.open(*txt_file)
...
def filter_null_rows(df, pipeline_name):
    if pipeline_name == "physicians_and_other_supplier_2012" or pipeline_name == "physicians_and_other_supplier_2013" or pipeline_name == "physicians_and_other_supplier_2014" or pipeline_name == "physicians_and_other_supplier_2015":

        df = df.query('npi != "" | place_of_service !="" | hcpcs_code !="" ')
