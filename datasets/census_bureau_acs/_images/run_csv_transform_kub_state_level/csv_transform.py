...
def main(
    ...
    df = extract_data_and_convert_to_df(group_id,state_code, year_report,api_naming_convention,geography)
    ...
    elif geography == 'schooldistrictelementary' or geography == 'schooldistrictsecondary' or geography == 'schooldistrictunified':
        df['geo_id'] = df['state'] + df['school_district']
    elif geography == 'cbsa' :
...
def extract_data_and_convert_to_df(group_id: dict, state_code: dict, year_report: str,api_naming_convention: str, geography: str) -> pd.DataFrame:
    list_temp = []
    for key in group_id:
