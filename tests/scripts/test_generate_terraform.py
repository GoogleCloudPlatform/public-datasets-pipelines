...
def test_main_generates_tf_files(
    ...
    assert (ENV_DATASETS_PATH / dataset_path.name / "_terraform" / "terraform.tfvars").exists()
...
def test_validation_on_generated_tf_files_in_dot_env_dir(
    ...
    subprocess.check_call(["terraform", "validate"], cwd=env_dataset_path / "_terraform")
...
def test_validation_on_generated_tf_files_in_project_dir(
    ...
    subprocess.check_call(["terraform", "init"], cwd=(project_dataset_path / "_terraform"))
    subprocess.check_call(["terraform", "validate"], cwd=(project_dataset_path / "_terraform"))
